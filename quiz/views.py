...
def NewQuiz(request, course_id, module_id):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			quiz = Quizzes.objects.create(user=user, title=title, description=description, due=due, allowed_attempts=allowed_attempts, time_limit_mins=time_limit_mins,points=points)
...
def NewQuestion(request, course_id, module_id, quiz_id):
	...
	if request.method == 'POST' and 'ask' in request.POST:
